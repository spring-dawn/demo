...
function ReportModal(props) {
    const { mode, data, close, render, allData, retry, sgg } = props;
    ...
    const insertToUpdate = (formData) => {
        if (formData !== undefined) {
            if (mode === "insert") {
                ...
                return axios
                    .post("/api/data/report", submitFormData, {
                        headers: {
                ...
                return axios
                    .patch("/api/data/report", JSON.stringify(submitData), {
                        headers: {
    ...
    useEffect(() => {
        if (del.del) {
            Swal.fire(msg.alertMessage["double_check"]).then((res) => {
                if (res.isConfirmed) {
                    fetch("/api/data/report?reportNo=" + del.id.reportNo, {
                        method: "DELETE",
                    })
                        .then((res) => {
                            if (res.ok) {
