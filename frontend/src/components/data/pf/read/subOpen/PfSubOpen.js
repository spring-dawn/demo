...
export default function PfSubOpen() {
    ...
    const excelDownload = () => {
        ...
        let url = "/api/data/facility/read/open/sub/standard/download?";
        url += `year=${year}`;
        url += `&month=${month}`;
        url += `&sggCd=${sggCd}`;
    ...
    const formatDownload = () => {
        if (!confirm("현재 접속 담당자의 소속에 따른 부설개방주차장 보고서 양식을 다운로드 합니다.")) return;
        axios("/api/data/facility/read/open/sub/standard/manager/keyword")
            .then((res) => {
    ...
    return (
        <div className="pageWrap">
            ...
            <Modal open={modalOpen} close={closeModal} header="부설개방주차장 현황">
