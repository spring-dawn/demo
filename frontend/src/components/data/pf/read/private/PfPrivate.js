...
export default function PfPrivate() {
    ...
    const columnDefs = [
        ...
        { field: "totalSpcs", headerName: "총주차면수", sortable: true },
    ];

    // 민영(노외)주차장 표준화 양식 엑셀 다운로드. location = useLocation() 이 같은 곳에 정의돼있으면 url 요청 안 들어감. 대안 필요.
    ...
    const excelDownload = () => {
        ...
        let url = "/api/data/facility/read/prv/standard/download?";
        url += `year=${year}`;
        url += `&month=${month}`;
        url += `&sggCd=${sggCd}`;
    ...
    const formatDownload = () => {
        if (!confirm("현재 접속 담당자의 소속에 따른 민영주차장 보고서 양식을 다운로드 합니다.")) return;
        axios("/api/data/facility/read/prv/standard/manager/keyword")
            .then((res) => {
    ...
    return (
        <div className="pageWrap">
            ...
            <div className="tableWrap">
                <div className="ag-theme-alpine" style={{ width: "100%", height: "490px" }}>
                    <AgGridReact
                        headerHeight={40}
                        rowData={data}
            ...
            <Modal open={modalOpen} close={closeModal} header="민영주차장 현황">
